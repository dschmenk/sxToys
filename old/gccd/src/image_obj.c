...
#endif
                *(unsigned short *)dst = pixel ^ sign_bit;
                src += 2;
                dst += 2;
            }
            break;
        case 4:
            while (count--)
            {
                ...
                pixel = ((pixel & 0xFF000000) >> 24)
                      ...
                      | ((pixel & 0x000000FF) << 24);
#endif
                *(unsigned long *)dst = pixel ^ sign_bit;
                src += 4;
                dst += 4;
            }
            break;
...
int ccd_image_load_fits(struct ccd_image *image)
{
    ...
    if ((fd = open(filename, O_RDONLY, 0)) > 0)
    {
        ...
        for (i = 0; i < image->height; i++)
        {
            ...
            convert_pixels(fits_pixels, image->pixels + image_size - (i+1) * image_pitch, image->zero == 0.0 ? 0 : 1 << (pixel_size*8-1), pixel_size, image->width);
        }
        free(fits_pixels);
        close(fd);
